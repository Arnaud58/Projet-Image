...
photo_fill.PhotoFillPixelsFromDatasetTask=function(dataset,opt_options){
  ...
  //this.similarity_task=new pixels_similarity.HistoColorSimilarityTask(_dataset,{});
  //this.similarity_task=new pixels_similarity.GridHistoColorSimilarityTask(_dataset,{});
  //this.similarity_task=new pixels_similarity.GridMeanRGBASimilarityTask(_dataset,{});
;
}
...
photo_fill.PhotoFillPixelsFromDatasetTask.prototype.process=function(in_imageData,out_imageData) {
  ...
  for (var y=0;y<in_imageData.height;y++) {
    for (var x=0;x<in_imageData.width;x++,w+=4) {
      if (in_imageData.data[w+3]==0) continue;
